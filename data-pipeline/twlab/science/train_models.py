...
from .forecast import forecast_benchmark
...
def main():
    ...
    # --- forecaster experiment (accelerating MC + false-alarm control bank), a module function so
    #     it is the single source of truth and can be regenerated without the torch lane (#24) ---
    forecast = forecast_benchmark(daysList)
    print(f"forecast: detect {forecast['detectRate']} | medErr {forecast['medErrPct']}% "
          f"| falseAlarm {forecast['falseAlarmRate']} ({forecast['nControl']} controls) "
          f"| leadCurve {[ (b['lo'],b['medErr']) for b in forecast['leadCurve']]}")
