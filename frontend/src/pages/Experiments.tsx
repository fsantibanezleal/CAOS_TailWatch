  ...
  return (
    <div className="page-body prose">
      <div className="page-head">
        ...
        <p className="lede">{es ? 'El diseño experimental: el split sin fuga, la cobertura de casos × clases, y el experimento de pronóstico por velocidad inversa con sus resultados reales.' : 'The experimental design: the leakage-safe split, the case × class coverage, and the inverse-velocity forecasting experiment with its real results.'}</p>
      </div>
      <section>
        ...
        {!m ? <p className="tw-hint">{es ? 'Cargando resultados…' : 'Loading results…'}</p> : (() => {
          ...
          return (<>
            ...
            <LeadChart f={f} es={es} />
            <p className="tw-note">{es ? `Sobre ${f.nTraj} escenas acelerantes no vistas, la velocidad inversa (Fukuzono) recupera el tiempo de falla al ${f.medErrPct}% mediano, pero la exactitud depende del adelanto: cae a ~2% dentro de 40 días de la falla y se ensancha lejos. Un pronóstico accionable solo emerge a medida que la falla se acerca. La serie se promedia en un parche y se filtra en el tiempo (el APS es el ruido principal del InSAR).` : `Over ${f.nTraj} unseen accelerating scenes, inverse velocity (Fukuzono) recovers the failure time to ${f.medErrPct}% median, but accuracy is lead-time-dependent: it falls to ~2% within 40 days of failure and widens far out. An actionable forecast only emerges as failure approaches. The series is patch-averaged and temporally filtered (APS is the dominant InSAR noise).`}</p>
            {f.falseAlarmRate != null && f.nControl != null && (
