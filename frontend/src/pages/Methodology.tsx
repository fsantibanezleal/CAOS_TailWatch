...
function Sbas({ es }: { es: boolean }) {
  ...
  return (<>
    ...
    <p>{es ? 'Honesto: esta pestaña ENSEÑA la inversión; el pipeline de TailWatch no la ejecuta. El modelo forward emite directamente el desplazamiento por época que esa inversión recuperaría, y el motor corre la descomposición 2-geometrías + la velocidad OLS sobre él. Ejecutar la inversión de red es roadmap.' : 'Honest: this tab TEACHES the inversion; the TailWatch pipeline does not run it. The forward model emits directly the per-epoch displacement that inversion would recover, and the engine runs the 2-geometry decomposition + OLS velocity on it. Running the network inversion is roadmap.'}</p>
...
function Learned({ es }: { es: boolean }) {
  return (<>
    ...
    <p>{es ? 'Un autoencoder denoising se entrena SOLO con parches normales: corrompe la entrada y reconstruye el parche limpio. Con cuello de botella estrecho reconstruye mal lo anómalo → el error es un puntaje de anomalía sin etiquetas.' : 'A denoising autoencoder trains ONLY on normal patches: it corrupts the input and reconstructs the clean patch. With a tight bottleneck it reconstructs anomalies poorly → the error is a label-free anomaly score.'}</p>
