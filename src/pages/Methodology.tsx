...
function Learned({ es }: { es: boolean }) {
  return (<>
    ...
    <p>{es ? 'Un AE simple puede reconstruir también las anomalías (atajo de identidad; el cuello solo no basta — Bouman & Heskes 2025); el denoising lo mitiga (MemAE, Gong 2019, es el paso siguiente). El AE convolucional para InSAR está establecido (Rouet-Leduc 2021). El CNN 1-D (estilo WDCNN) clasifica el tipo de deformación; Grad-CAM muestra dónde mira.' : 'A plain AE can reconstruct anomalies too (identity shortcut; the bottleneck alone is insufficient — Bouman & Heskes 2025); denoising mitigates it (MemAE, Gong 2019, is the next step). The convolutional AE for InSAR is established (Rouet-Leduc 2021). The 1-D CNN (WDCNN-style) classifies the deformation type; Grad-CAM shows where it attends.'}</p>
